...
model_server_url = config.get("model_server_url", "http://localhost:9000")
serpapi_key = config.get("serpapi_key", "")

agent = ChatbotAgent(model_server_url, serpapi_key)
...
async def chat(query: Query):
    ...
    response = agent.handle_query(query.text)
    bias_score = detect_bias(response)
    if bias_score > 0.5:
